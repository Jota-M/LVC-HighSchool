...
import { useMisMaterias, useBoletinEstudiante } from '@/hooks/useEstudiante';
      ...
      <CardContent sx={{ p:2.5, pb: expandido ? 1 : 2.5 }}>
        ...
        >
          ...
          <Box sx={{ flex:1, minWidth:0 }}>
            ...
            {(['SER','SAB','HAC'] as const).map((cod, i) => (
              <BarraDimension
                key={cod}
                codigo={cod}
                nota={cod === 'SER' ? item.nota_ser : cod === 'SAB' ? item.nota_saber : item.nota_hacer}
                delay={index * .06 + i * .05}
              />
            ))}
          </Box>
...
export const EstudianteCalificaciones: React.FC<Props> = () => {
  ...
  // Extraer períodos únicos de las materias
  const periodos = useMemo(() => {
    const map = new Map<number, string>();
    for (const m of materias) {
      if (m.periodo_evaluacion_id && m.trimestre_nombre) {
        map.set(m.periodo_evaluacion_id, m.trimestre_nombre);
      }
    }
  ...
  useEffect(() => {
    if (periodos.length > 0 && periodoActivo === null) {
  ...
  const isLoading = loadingMaterias || loadingBoletin;
