// components/estudiantes/registro/ModoRegistroSelector.tsx
'use client';
...
import { PadreEncontrado } from '@/types/estudianteTypes';
...
export default function ModoRegistroSelector({ onModoSeleccionado }: ModoRegistroSelectorProps) {
  ...
  const handleBuscarPadre = async () => {
    ...
    try {
      const data = await registroCompletoService.buscarPadrePorCI(ciBusqueda);
  ...
  const handleContinuar = () => {
    if (modo === 'existente' && !padreEncontrado) {
  ...
  const cardStyle = {
    ...
    '&:hover': {
      ...
      boxShadow: isDark
        ? '0 20px 60px rgba(250, 204, 21, 0.3)'
      ...
      <Container maxWidth="lg">
        <Fade in timeout={500}>
          <Box>
            ...
            {/* Cards de Opciones */}
            <Box
              sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 3, mb: 4 }}
            >
              {/* Opción 1: Nuevo */}
              <Card
                sx={{
              ...
              >
                <CardContent sx={{ p: 4, textAlign: 'center' }}>
                  <Box sx={iconBoxStyle('#10b981', modo === 'existente')} mx="auto" mb={3}>
                    <FamilyRestroomIcon
                      sx={{ fontSize: 40, color: modo === 'existente' ? '#fff' : '#10b981' }}
                    />
                  </Box>
                  <Radio
                    value="existente"
            ...
            {/* Búsqueda de Padre (solo si existente) */}
            {modo === 'existente' && (
              <Fade in timeout={300}>
                <Paper
                  elevation={0}
                      ...
                      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2, mb: 2 }}>
                        <Box>
                          ...
                          <Typography variant="body2" fontWeight={600}>
                            {padreEncontrado.nombres} {padreEncontrado.apellido_paterno}{' '}
                            {padreEncontrado.apellido_materno}
                          </Typography>
                        </Box>
                        <Box>
                          <Typography variant="caption" color="text.secondary">
            ...
            <Box sx={{ textAlign: 'center' }}>
              <Button
                ...
                disabled={modo === 'existente' && !padreEncontrado}
                sx={{
                  py: 2,
                  px: 6,
                  fontSize: '1.2rem',
              ...
              >
                ...
                {modo === 'existente' && 'Continuar con Padre Existente'}
