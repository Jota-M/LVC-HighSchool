...
export const useMisAsignaciones = () => {
  ...
  const cargar = useCallback(async (fechaTarget?: string) => {
    ...
    try {
      ...
      // 404 = docente sin asignaciones activas — no es un error crítico
  ...
  // Marcar localmente una asignación como completa (optimistic update
  // después de guardar el pase de lista, sin re-fetch)
      ...
      prev.map(a =>
        a.asignacion_id === asignacion_id
          ? {
              ...
              total_marcados: a.total_estudiantes,
              total_pendientes: 0,
            }
          : a
      )
