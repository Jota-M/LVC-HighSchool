'use client';
// app/dashboard/padre/pagos/[estudianteId]/pagar/[mensualidadId]/page.tsx
...
import { useQRPago } from '@/hooks/usePadrePagos';
import { formatFechaPago } from '@/types/padrePagosTypes';
...
// ─── Paso de instrucción ──────────────────────────────────────────────────────
const Paso: React.FC<{ num: number; texto: string; accent: string; isDark: boolean; delay?: number }> = ({
...
// ─── Página principal ─────────────────────────────────────────────────────────
export default function PagarMensualidadPage() {
  const { isDark, gold, goldEnd, gradBg } = usePalette();
  const router = useRouter();
  const params = useParams();

  // padre-pagar-qr-page.tsx
  const estudianteId  = Number(params.id);
  const mensualidadId = Number(params.mid);
  ...
  const {
    ...
    cancelarQR,
    verificarEstado,
  } = useQRPago(mensualidadId || null);

  // Cuando se confirma el pago, redirigir después de 3 segundos
  useEffect(() => {
    if (pagado) {
      const timer = setTimeout(() => {
        router.push(`/dashboard/padre/pagos/${estudianteId}`);
      }, 3500);
          ...
          <Fade in timeout={500}>
            <Box sx={{ textAlign: 'center', animation: `${fadeUp} 0.5s ease-out` }}>
              ...
              <Typography variant="body1" color="text.secondary" sx={{ mb: 1 }}>
                Tu mensualidad de{' '}
                <strong>{qrData?.mes}</strong> fue pagada exitosamente.
              </Typography>
              {qrData && (
              ...
              <Box sx={{ mt: 3 }}>
                <Box
                  onClick={() => router.push(`/dashboard/padre/pagos/${estudianteId}`)}
                  sx={{
        ...
        <Box sx={{ pt: 3, pb: 6 }}>
          ...
          <Fade in timeout={500}>
            <Box sx={{
              ...
              <Box sx={{
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                p: 4, minHeight: 320, position: 'relative',
              }}>
                ...
                {isGenerando && (
                  <Box sx={{ textAlign: 'center', animation: `${fadeUp} 0.3s ease-out` }}>
                    <CircularProgress
                      size={48}
                      sx={{ color: isDark ? gold : '#d97706', mb: 2 }}
                    />
                    <Typography variant="body2" color="text.secondary" fontWeight={600}>
                      Generando tu QR...
                    </Typography>
              ...
              {/* ── Footer del QR: vencimiento + acciones ── */}
              {qrData && !isGenerando && (
                <Box sx={{
                  px: 3, py: 2,
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                    ...
                    <Typography variant="caption" color="text.secondary" fontWeight={600} sx={{ fontSize: 11 }}>
                      Vence: {formatFechaPago(qrData.qr_expiracion)}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    ...
                    {/* Cancelar QR */}
                    <Tooltip title="Cancelar este QR">
                      <Box
                        onClick={cancelarQR}
                        sx={{
                          display: 'flex', alignItems: 'center', gap: 0.6,
                      ...
                      >
                        {isCancelando ? (
                          ...
                          <CancelRoundedIcon sx={{ fontSize: 13 }} />
                        )}
                        Cancelar
                      </Box>
                    </Tooltip>
