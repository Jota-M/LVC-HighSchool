...
import {
  Box, Typography, Chip, Skeleton, Fade, alpha, useTheme, LinearProgress,
} from '@mui/material';
...
function agruparPorMateria(asignaciones: AsignacionDocente[]) {
  ...
  asignaciones.forEach(a => {
    if (!map.has(a.materia_nombre)) map.set(a.materia_nombre, []);
    map.get(a.materia_nombre)!.push(a);
  });
...
const AsignacionCard: React.FC<{
  ...
  const totalMateriales = (asignacion as any).total_materiales ?? null;
  const hayStats = totalMateriales !== null;
    ...
    >
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mb: 2 }}>
        <Box sx={{ bgcolor: isDark ? alpha('#fff', 0.04) : alpha('#f8f9fa', 0.8), borderRadius: '10px', p: 1.2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, mb: 0.3 }}>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ fontSize: 10 }}>Grado</Typography>
          </Box>
          <Typography variant="body2" fontWeight={800} noWrap>{asignacion.grado_nombre}</Typography>
        </Box>

        <Box sx={{ bgcolor: isDark ? alpha('#fff', 0.04) : alpha('#f8f9fa', 0.8), borderRadius: '10px', p: 1.2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, mb: 0.3 }}>
            <GroupsIcon sx={{ fontSize: 13, color: 'text.disabled' }} />
            <Typography variant="caption" color="text.secondary" sx={{ fontSize: 10 }}>Estudiantes</Typography>
          </Box>
      ...
      <Box sx={{ mb: 2 }}>
        <Chip
          ...
          size="small"
          icon={<BookmarkIcon sx={{ fontSize: '11px !important', color: `${gold} !important` }} />}
          sx={{
            height: 22, fontSize: '0.68rem', fontWeight: 700,
            bgcolor: alpha(gold, isDark ? 0.1 : 0.08),
            color: gold,
        ...
        <Fade in timeout={500}>
          <Box sx={{ mb: 4 }}>
            ...
            <Typography variant="body1" color="text.secondary" sx={{ fontWeight: 500 }}>
              ...
              Seleccioná una materia para gestionar sus recursos académicos.
            </Typography>
          </Box>
        </Fade>
                ...
                {loading
                  ...
                  : grupo.map(asig => (
                    <AsignacionCard
                      ...
                      goldEnd={goldEnd}
                      isDark={isDark}
                      onClick={() => router.push(`/dashboard/docente/materiales/${asig.asignacion_id}`)}
                    />
                  ))
                }
              </Box>
