'use client';
// components/DashboardTopBar.tsx — con NotificacionCampana integrada
  ...
  return (
    ...
    >
      <Toolbar sx={{ justifyContent: 'space-between' }}>
        <Typography variant="h6" sx={{ fontWeight: 700, color: isDark ? '#fff' : '#263238' }} />

        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>

          {/* ── Campana (reemplaza al IconButton anterior) ── */}
          <NotificacionCampana />

          {/* Configuración */}
          <IconButton sx={{
            color: isDark ? '#b0bec5' : '#607d8b',
            '&:hover': { bgcolor: isDark ? alpha('#0288d1', 0.1) : alpha('#0288d1', 0.05) },
          }}>
          ...
          <IconButton
            ...
            sx={{
              color: isDark ? alpha('#fff', 0.8) : '#6b7280',
              '&:hover': { bgcolor: isDark ? alpha('#fff', 0.08) : '#f3f4f6', color: isDark ? '#fff' : '#111827' },
          ...
          {/* Avatar */}
          <IconButton onClick={handleMenu} sx={{ p: 0, ml: 1, '&:hover': { transform: 'scale(1.05)' } }}>
            <Avatar src="/perfil.jpg" sx={{ width: 40, height: 40, border: '2px solid #0288d1' }} />
          </IconButton>
              ...
              <Box sx={{ mt: 1 }}>
                {user?.roles?.map((role: any) => (
                  <Typography key={role.id} variant="caption" sx={{
                    display: 'inline-block', px: 1, py: 0.5, mr: 0.5,
                    borderRadius: 1, bgcolor: alpha('#0288d1', 0.1),
