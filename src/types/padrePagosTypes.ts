...
export interface MensualidadHijo {
  ...
  // Pago asociado (si existe), sin importar el método
  pago_id: number | null;
  codigo_pago: string | null;
  metodo_pago: 'efectivo' | 'transferencia' | 'qr' | 'tarjeta' | null;
...
// =============================================
// GRUPO DE PAGO — cuotas que salieron de una misma
// transacción (mismo QR, o mismo lote cargado por el admin)
// =============================================
export interface GrupoPagoCuota {
  mensualidad_id: number;
  numero_cuota: number;
  mes_correspondiente: string;
  pago_id: number;
...
export interface GrupoPago {
  clave: string;              // transaccion_id, o `pago-{id}` si no hay grupo real
  transaccion_id: string | null;
  metodo_pago: 'efectivo' | 'transferencia' | 'qr' | 'tarjeta' | null;
  fecha_pago: string | null;
  monto_total: number;
  pago_ids: number[];
  cuotas: GrupoPagoCuota[];
}
